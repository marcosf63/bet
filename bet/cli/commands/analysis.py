...
def analysis_command(
    arquivo: str = typer.Option(
        "notebooks/dados/lucro_por_operacao.csv", help="Caminho para o arquivo de dados CSV."
    ),
    coluna: str = typer.Option("Lucro_Lay", help="Nome da coluna com os retornos/lucros."),
    salvar_graficos: bool = typer.Option(False, "--salvar", help="Salvar gráficos em arquivo PNG."),
    simulacoes: int = typer.Option(10000, help="Número de simulações Monte Carlo."),
    operacoes_futuras: int = typer.Option(500, help="Número de operações para simular no futuro."),
):
    ...
    if not ANALYTICS_AVAILABLE:
        ...
        console.print(
            "[yellow]💡 Instale as dependências: pip install scipy matplotlib seaborn[/yellow]"
        )
        return

    console.print("[bold blue]📊 Iniciando análise da estratégia...[/bold blue]")
    ...
    try:
        ...
        if not arquivo_path.exists():
            ...
            if arquivo_path.exists():
                ...
                console.print(
                    "[yellow]💡 Verifique o caminho do arquivo ou use o comando com --help[/yellow]"
                )
                return
        ...
        console.print(
            f"[blue]📈 Mínimo/Máximo:[/blue] {basic_stats['min']:.4f} / {basic_stats['max']:.4f}"
        )
        ...
        with console.status("[bold green]Executando simulações..."):
            mc_results = analyzer.monte_carlo_simulation(
                n_simulations=simulacoes, n_periods=operacoes_futuras
            )
        ...
        console.print(
            f"[yellow]📊 Cenário otimista (95%):[/yellow] {mc_results['percentile_95']:.2f}"
        )
        ...
        if perf_metrics["expectancy"] > 0:
            console.print(
                "[green]✅ Estratégia com expectância positiva - continuar execução[/green]"
            )
        else:
            console.print(
                "[red]❌ Estratégia com expectância negativa - revisar urgentemente[/red]"
            )
        ...
        console.print(
            f"[cyan]🎯 Sugestão de sizing: {kelly*100:.1f}% do capital por operação[/cyan]"
        )
