...
from bet.cli.helpers import get_daily_games, filter_games, safe_float, save_csv_to_file, print_isc_legend
...
def layaway_command(
    ...
    if odd_home_max is not None:
        jogos_do_dia = [
            ...
            if safe_float(jogo.get("Odd_H_Back")) <= odd_home_max
        ]

    if odd_home_min is not None:
        jogos_do_dia = [
            jogo for jogo in jogos_do_dia
            if safe_float(jogo.get("Odd_H_Back")) >= odd_home_min
        ]
