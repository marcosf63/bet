...
def diff_command(
    ...
    console.print(f"[bold cyan]Busca correspondencias entre MDAY e SOFA[/bold cyan]")
    ...
    try:
        ...
        if odd_home_max is not None:
            jogos_do_dia = [j for j in jogos_do_dia if safe_float(j.get(odd_home_col)) <= odd_home_max]
        
        if odd_home_min is not None:
            jogos_do_dia = [j for j in jogos_do_dia if safe_float(j.get(odd_home_col)) >= odd_home_min]
            ...
            for jogo in jogos_mday:
                ...
                if isc_val_str != "N/A":
                    try:
                        ...
                        if ((isc_lower == "excelente" and isc_val > 75) or
                            (isc_lower == "bom" and isc_val >= 65) or
                            (isc_lower == "moderado" and isc_val >= 55) or
                            (isc_lower == "fraco" and isc_val < 55)):
        ...
        if status:
            eventos_sofa = [e for e in eventos_sofa if 
                           status.lower() in e.status.type.lower() or 
                           status.lower() in e.status.description.lower()]
        ...
        for jogo_mday in jogos_mday:
            ...
            for jogo_sofa in jogos_sofa:
                ...
                # Similaridade
                    ...
                    home_sim = calculate_similarity(jogo_mday.get("Home", ""), jogo_sofa.get("Home", ""))
                    away_sim = calculate_similarity(jogo_mday.get("Away", ""), jogo_sofa.get("Away", ""))
        ...
        if jogos_correspondentes:
            ...
            # Estatisticas
            lucros_lay = [float(j.get("Lucro Lay Away", "0")) for j in jogos_correspondentes 
                          if j.get("Lucro Lay Away", "N/A") != "N/A"]
            lucros_back = [float(j.get("Lucro Back Home", "0")) for j in jogos_correspondentes 
                           if j.get("Lucro Back Home", "N/A") != "N/A"]
            
            console.print(f"\n[bold]Estatisticas:[/bold]")
            if lucros_lay:
                console.print(f"LAY AWAY: {len(lucros_lay)} jogos, lucro total: {sum(lucros_lay):.3f}")
            if lucros_back:
                console.print(f"BACK HOME: {len(lucros_back)} jogos, lucro total: {sum(lucros_back):.3f}")
            
            if salvar:
                filtros = [fonte]
                if odd_home_max: filtros.append(f"home-max-{odd_home_max}")
                if odd_home_min: filtros.append(f"home-min-{odd_home_min}")
                if isc: filtros.append(f"isc-{isc.lower()}")
                if liga: filtros.append(f"liga-{liga.lower().replace(' ', '-')}")
                if status: filtros.append(f"status-{status.lower()}")
                save_diff_to_file(jogos_correspondentes, data, filtros)
        else:
            console.print(f"[yellow]Nenhuma correspondencia encontrada[/yellow]")
