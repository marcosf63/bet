...
def calcula_saida_edge_black_lay(odd_entrada_back: float, odd_saida_lay: float, stake: float):
    """
...
def verificar_tempo_passado(hora_passada: datetime, minutos: int) -> bool:
    ...
    return diferenca_tempo > timedelta(minutes=0) and diferenca_tempo < timedelta(minutes=minutos)
...
def normalizar_nome_equipe(nome: str) -> str:
    ...
    nome_normalizado = "".join(
        char for char in nome_normalizado if unicodedata.category(char) != "Mn"
    )
...
def encontrar_correspondencias(
    jogos_mday: list[dict], jogos_sofa: list[dict], limiar_similaridade: float = 0.6
) -> dict:
    """
    ...
    for idx_mday, jogo_mday in enumerate(jogos_mday):
        ...
        for idx_sofa, jogo_sofa in enumerate(jogos_sofa):
            ...
            if (
                similaridade_total > melhor_similaridade
                and similaridade_total >= limiar_similaridade
            ):
    ...
    jogos_unicos_mday = [
        jogo for idx, jogo in enumerate(jogos_mday) if idx not in indices_mday_usados
    ]

    jogos_unicos_sofa = [
        jogo for idx, jogo in enumerate(jogos_sofa) if idx not in indices_sofa_usados
    ]
...
def gerar_relatorio_comparacao(resultado_comparacao: dict) -> None:
    ...
    taxa_correspondencia = (
        (len(correspondencias) / max(total_mday, total_sofa)) * 100
        if max(total_mday, total_sofa) > 0
        else 0
    )
...
async def interceptar_requisicoes_http(url: str, tempo_espera: int = 10000) -> list:
    ...
    async with async_playwright() as p:
        ...
        async def capture_response(response):
            if (
                "/api/" in response.url
                and "sofascore.com" in response.url
                ...
                try:
                    ...
                    if "application/json" in content_type:
                        ...
                        try:
                            ...
                            print(
                                f"[green]✅ API JSON válida: {response.url} ({response_size} bytes)[/green]"
                            )
                        except:
                            print(f"[green]✅ API JSON válida: {response.url}[/green]")
                    else:
                        print(
                            f"[yellow]⚠️  API não-JSON: {response.url} (content-type: {content_type})[/yellow]"
                        )
        ...
        try:
            print("[dim]🌐 Navegando para a página...[/dim]")
            await page.goto(url, wait_until="load", timeout=30000)

            print(
                f"[dim]⏱️ Aguardando {tempo_espera/1000}s para capturar todas as requisições...[/dim]"
            )
...
def testar_interceptador_sofascore(
    url: str = "https://www.sofascore.com/pt/football/match/fulham-chelsea/NsT#id:14025097",
) -> None:
    """
    ...
    console.print("[bold cyan]🧪 TESTE: Interceptando APIs do SofaScore[/bold cyan]")
    ...
    console.print("   • Domínio: [yellow]sofascore.com[/yellow]")


def comparar_mday_sofa(
    data: str = None, fonte_mday: str = "betfair", limiar_similaridade: float = 0.6
) -> dict:
    """
...
def buscar_resultado_partida(
    home_team: str, away_team: str, data: str, primeiro_tempo: bool = False
) -> dict:
    """
    ...
    try:
        ...
        if primeiro_tempo:
            query = (
                f"{home_team} vs {away_team} {data_formatada} resultado intervalo HT primeiro tempo"
            )
        else:
