...
def get_scheduled_events(target_date: str = None) -> List[ScheduledEvent]:
    ...
    try:
        ...
        if resp.status_code == 403:
            print(f"[red]⚠️  API SofaScore retornou 403 (Forbidden)[/red]")
            print(f"[yellow]🔄 Tentando acessar via Playwright...[/yellow]")
            
            # Tentar com Playwright como fallback
            if PLAYWRIGHT_AVAILABLE:
                return _get_scheduled_events_playwright(target_date)
            else:
                print(f"[red]❌ Playwright não disponível. Instale com: pip install playwright[/red]")
                print(f"[yellow]💡 Depois execute: playwright install chromium[/yellow]")
                return []
        elif resp.status_code == 429:
            print(f"[red]⚠️  Rate limit excedido (429)[/red]")
            print(f"[yellow]💡 Aguarde alguns minutos antes de tentar novamente[/yellow]")
            return []
        ...
        for event_data in events_list:
            try:
                ...
                print(f"[red]Erro ao processar evento {event_data.get('id', 'desconhecido')}: {e}[/red]")
                continue
        ...
        print(f"[red]⏰ Timeout ao acessar API SofaScore[/red]")
        return []
    except requests.exceptions.ConnectionError:
        print(f"[red]🔌 Erro de conexão com API SofaScore[/red]")
        return []
...
async def _fetch_with_playwright(target_date: str) -> List[ScheduledEvent]:
    ...
    async with async_playwright() as p:
                ...
                try:
                    ...
                    if 'application/json' in content_type:
                        ...
                        if "events" in json_data:  # Verificar se tem eventos
                            ...
                            print(f"[green]✅ JSON com eventos capturado da API via interceptação[/green]")
                        else:
                            print(f"[yellow]📅 API interceptada mas sem eventos para {target_date}[/yellow]")
                except Exception as e:
        ...
        try:
            ...
            print(f"[dim]📊 Aguardando carregamento das APIs...[/dim]")
            ...
            try:
                ...
                if not captured_response:
                    print(f"[dim]⏱️ Aguardando mais tempo para APIs...[/dim]")
            ...
            if captured_response and "events" in captured_response:
                ...
                print(f"[green]✅ Playwright: Processados {len(scheduled_events)} eventos para {target_date}[/green]")
                return scheduled_events
            else:
                print(f"[yellow]⚠️  API não foi interceptada. Tentando acesso direto...[/yellow]")
                    ...
                    if text_content.strip().startswith('{'):
                        try:
                            ...
                            if "events" in events_data:
                                ...
                                target_datetime = datetime.strptime(target_date, "%Y-%m-%d")
                                target_timestamp_start = int(target_datetime.timestamp())
                                target_timestamp_end = int((target_datetime.timestamp() + 24 * 60 * 60))
                                
                                print(f"[dim]Processando {len(events_list)} eventos via acesso direto...[/dim]")
                                ...
                                print(f"[green]✅ Playwright (direto): Encontrados {len(scheduled_events)} eventos para {target_date}[/green]")
                                return scheduled_events
                            else:
                                print(f"[yellow]Nenhum evento encontrado para {target_date}[/yellow]")
                                return []
